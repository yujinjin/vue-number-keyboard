		...
		<div class="page content">
			...
			<div class="verification-code-input">
				...
				<div class="verification-code-box" @click.stop.prevent="closeNumberKeyboard(2)">
					<span v-for="i in 6" class="key-box" :class="{'active': (i-1 == validateCode.length && isShowVerificationCodeKeyboard), 'used': validateCode.length > i-1}">
						<i class="icon-blink" v-if="i-1 == validateCode.length"></i>
						<template v-else="validateCode.length > i-1">{{validateCode.substr(i-1, 1)}}</template>
					</span>
				</div>
			...
			<div class="password-input">
				...
				<div class="password-box" @click.stop.prevent="closeNumberKeyboard(3)">
					<span v-for="i in 6" class="key-box" :class="{'active': (i-1 == password.length && isShowPasswordKeyboard), 'used': password.length > i-1}">
						<i class="icon-blink" v-if="i-1 == password.length"></i>
						<i class="icon-dot" v-else="password.length > i-1"></i>
					</span>
				</div>
...
<style lang="less">
	...
	.view {
		...
		.page {
	        ...
	        .verification-code-input, .password-input {
				...
				.verification-code-box, .password-box {
					position: relative;
					display: flex;
